...
# and recover the normal from cPhysicsRayParams. PhysicsWorldNewton's exact ray
# callback lays out mfDist at +4 and mvNormal at +8 before passing the same base
# to OnIntersect, so mvNormal.y is +0x0C. No second ray/solver is introduced.
...
Assert-Bytes 0x189D66 @(0xD9,0x05,0xF8,0xDD,0x6E,0x00,0xD8,0x4C,0x24,0x20,0xD9,0x1D,0x2C,0xDE,0x6E,0x00)
Assert-Bytes 0x189D80 @(0x8B,0x4C,0x24,0x14,0x8B,0x11,0x89,0x15,0x30,0xDE,0x6E,0x00,0x8B,0x51,0x04,0x89,0x15,0x34,0xDE,0x6E,0x00,0x8B,0x49,0x08,0x89,0x0D,0x38,0xDE,0x6E,0x00)
Assert-Bytes 0x189E0C @(0x68,0x28,0xDE,0x6E,0x00,0x50,0xFF,0x52,0x04)
# cCharacterBodyCollidePush is the +0x224 callback (vtable 0x67F7AC). Its
# OnCollision entry at D5A60 rejects contacts when both CharacterBody +0x70
# and +0x74 are zero, compares the contacted body's mass against +0x98 and
# multiplies its contact-direction force by CharacterBody +0x9C. The VR
# adapter temporarily supplies Rework's missing vr_velocity movement signal
# and 0.2x push-force semantics only around the existing D7312 solver call.
Assert-Bytes 0x27F7AC @(0x60,0x5A,0x4D,0x00)
Assert-Bytes 0xD5A6C @(0x8B,0x4F,0x04,0xD9,0x41,0x70)
Assert-Bytes 0xD5A7F @(0xD9,0x05,0xC0,0x29,0x67,0x00,0xD9,0x41,0x74)
Assert-Bytes 0xD5AB4 @(0x8B,0x4F,0x04,0x8B,0x91,0x98,0x00,0x00,0x00)
Assert-Bytes 0xD5C8A @(0x8B,0x43,0x04,0xDD,0xD8,0xD9,0x80,0x9C,0x00,0x00,0x00)
...
# that can discard a physical-only winner when it is dynamic or fails Rework's
# upward-normal step gate before the native second pass.
...
Write-Output 'Verified player character-body Move, construction, feet, physics-world Update, pre-collision physical X/Z injection, native dynamic-body push callback fields, character step-ray callback/vtable, ray-normal ABI and Rework-equivalent physical step boundary. No process was modified.'
