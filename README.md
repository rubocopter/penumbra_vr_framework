...
The native player-body/collision boundary, first narrow body adapter, default-off tracking/body reconciliation shadow and separate **collision-aware physical X/Z displacement request** through the existing native tick are live-tested. PID 26144 demonstrated queue/injection/native collision/matched reconciliation at the exact-build pre-collision boundary. PID 24956 then exercised the active camera/body path and native crouch recovery in a headset, but reported unintended character walking when the head was only tilted in place. Static comparison also found double accounting between the combined Black Plague body tick and Rework's locomotion-only anchor carry. That partition is corrected and compiled, but whether it resolves the reported tilt behavior is untested; a repeat of `tools/Start-BlackPlagueRoomScaleValidation.ps1` remains required before headset validation.
