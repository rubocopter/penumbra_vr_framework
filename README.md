...
The native player-body/collision boundary, first narrow body adapter, default-off tracking/body reconciliation shadow and separate **collision-aware physical X/Z displacement request** through the existing native tick are live-tested. PID 26144 demonstrated queue/injection/native collision/matched reconciliation at the exact-build pre-collision boundary. PID 21548 later proved the combined-tick carry correction and removed locomotion from in-place head tilt, but exposed continuous world shake. PID 13672 headset-exercised the subsequent render-rate anchor correction and reported that shake gone. PID 11804 then confirmed stable general comfort and correct HMD-relative direction, while exposing native forward/back/side speed asymmetry when the hidden body orientation differed from the head. The next build routes Rework's direct `1.5/2.25 m/s` stick displacement through the existing `0xD7281` collision owner, keeps the combined physical/locomotion request within `0.05 m`, and reconciles both components separately. This is implemented and host-tested; active room-scale still awaits another headset run. Physical crouch-by-height and tracking-world-yaw turn ownership remain separate milestones.
