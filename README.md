...
The native player-body/collision boundary, first narrow body adapter, default-off tracking/body reconciliation shadow and separate **collision-aware physical X/Z displacement request** through the existing native tick are live-tested. PID 26144 demonstrated queue/injection/native collision/matched reconciliation at the exact-build pre-collision boundary. PID 21548 later proved the combined-tick carry correction and removed locomotion from in-place head tilt, but exposed continuous world shake. PID 13672 headset-exercised the subsequent render-rate anchor correction; the user reported that the continuous shake was gone and comfort was substantially improved. Active room-scale is still not headset-validated because stick-forward heading had felt offset unless recentered. The next build removes the rendered/native camera from that remap and derives horizontal stick heading directly from the recenter tracking anchor and current HMD pose, matching Rework's head-relative direction boundary. That correction is implemented and host-tested but still awaits the next `tools/Start-BlackPlagueRoomScaleValidation.ps1` headset run. Physical crouch-by-height, Rework's direct `1.5/2.25 m/s` displacement and tracking-world-yaw turn ownership remain separate milestones; the room-scale gate uses the already mapped native crouch control only to verify body replacement/recovery.
