...
The native player-body/collision boundary, first narrow body adapter, default-off tracking/body reconciliation shadow and separate **collision-aware physical X/Z displacement request** through the existing native tick are live-tested. PID 26144 demonstrated queue/injection/native collision/matched reconciliation at the exact-build pre-collision boundary. A separate transient validation mode now applies the fresh reconciled X/Z result to camera, visibility and controller space; its affected Release targets compile, but no test executable has run and host/live/headset validation still requires `tools/Start-BlackPlagueRoomScaleValidation.ps1`.
