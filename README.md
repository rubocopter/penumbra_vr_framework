...
The native player-body/collision boundary, first narrow body adapter, default-off tracking/body reconciliation shadow and separate **collision-aware physical X/Z displacement request** through the existing native tick are live-tested. PID 8092 supplied headset evidence for Rework-style HMD-relative `1.5/2.25 m/s` stick locomotion through that same collision owner. PID 20520 then exposed two remaining comfort boundaries: the first physical-crouch integration did not reliably synchronize standing, and short physical X/Z movement could still feel like the world pulled back. The corrected build owns Rework's crouch latch/height policy in the shared runtime, applies Black Plague's exact native crouch state from its existing game-thread owner, and composes continuous tracked Y from the reconciled feet anchor. These latest crouch/Y changes are host-tested and await the focused headset gate; positional comfort, tracking-world-yaw turn ownership and final camera/body/footstep bob remain open.
