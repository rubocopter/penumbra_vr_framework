...
The native player-body/collision boundary, first narrow body adapter, default-off tracking/body reconciliation shadow and separate **collision-aware physical X/Z displacement request** through the existing native tick are live-tested. PID 26144 demonstrated queue/injection/native collision/matched reconciliation at the exact-build pre-collision boundary. PID 21548 later passed the complete active room-scale helper after the combined-tick carry correction, and in-place head tilt no longer moved the character. The headset result still failed the comfort gate because the world shook continuously and wall rejection felt aggressive. The next correction places horizontal rendering directly at the reconciled VR anchor and continues it from the latest HMD pose between the native 60 Hz body ticks, matching Rework's camera ownership more closely. It compiles but requires another `tools/Start-BlackPlagueRoomScaleValidation.ps1` session before headset validation.
