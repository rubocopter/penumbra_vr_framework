...
- Default-off Black Plague physical-displacement validation path at exact-build RVA `0xD7281`. It injects a one-shot bounded X/Z request (maximum `0.05 m`, Y zero) immediately before native horizontal collision comparison, preserves the original instructions and single `D6E00` owner, and records queue/injection/acceptance telemetry from a pre-injection baseline. This path is implemented and host-tested; live validation is still pending and positional HMD translation remains zero.
- `tools/Start-BlackPlaguePhysicalDisplacementValidation.ps1` verifies the supported initialized executable, launches through Steam, holds the transient physical-validation mutex and now fails closed unless the fresh probe log proves a non-zero queued plan, consumed/injected pre-collision request, matched reconciliation, injected body telemetry, the native `dt~=1/60` body tick and sampled stationary/free/block/slide-or-partial cases. It reports each captured case once while the game is still running so missing coverage can be corrected before exit. PID 18392 confirmed that an activation-only session is rejected rather than miscounted as live validation.
...
- Black Plague positional HMD translation remains disabled. The exact native body/collision boundary, first narrow body adapter and default-off no-write reconciliation shadow consumer are live-tested. A separate bounded collision-aware physical X/Z request at `0xD7281` is now implemented and host-tested through the existing native tick; its dedicated live stationary/free/block/slide validation is the next body gate before active room-scale can be enabled.
...
- The Black Plague physical-displacement boundary passes local Release build, all **30/30** root CTest tests and the exact-build input-map verifier, including the pre-collision `0xD7281` injection window. This validates the host implementation only; no game/SteamVR/headset evidence has yet promoted the active injection path to live-tested.
