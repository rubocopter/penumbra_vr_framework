...
- Hardened Black Plague exact-build hook lifecycle handling: partial installs are
  ...
  tests pass.
- When the Black Plague monitor mirror is disabled, continuous stereo clears the desktop backbuffer to black instead of leaving stale desktop contents that produced the reported growing white-point artifact. PID 26144 visually confirmed the desktop remained black with mirror disabled.
...
- Black Plague positional HMD translation remains disabled. The exact native body/collision boundary, first narrow body adapter, default-off no-write reconciliation shadow consumer and bounded collision-aware physical X/Z request at `0xD7281` are live-tested. The next body gate is headset validation of active room-scale/positional HMD translation through that proven request boundary.
