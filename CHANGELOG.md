...
- 2026-09-19: The combined Black Plague headset run supplied partial positive
  evidence for the current interaction/body stack: open-space room-scale,
  mirror-on, glowstick placement/toggle feedback, direct hand pushing and
  partial palm blocking/sliding were observed working. The same run isolated
  four remaining regressions instead of promoting the candidate: unreliable
  natural `Grab=6`/`Move=2` acquisition, occasional palm discontinuity across
  world-yaw turns, degraded thumb/little-finger articulation on the imported
  Rework hand mesh, and repeated native `+0.05 m` step cycles when room-scale
  translation coincided with low/dynamic obstacles. The current host candidate
  makes the physical palm overlap the primary nearby acquisition source, protects
  the fresh selection target from nudge, rebases palm history on yaw-epoch
  changes, preserves Black Plague's independent skeletal finger channels and
  stronger free-hand curl response, applies Rework's static/upward-normal step
  gate to every tick carrying physical room-scale displacement, and adapts
  Rework's `0.2x` dynamic-body push semantics around the existing BP horizontal
  solver. These fixes remain **host-tested only** pending the focused headset
  retest.

- 2026-09-18: Follow-up live evidence split the current Black Plague startup/
  presentation regression into two independent failures. PID 14024 reproduced
  the same early `SDL.dll+0x28c09` access violation even with the visible
  `SDL_app` readiness predicate, proving that window stability alone is not a
  sufficient graphics-readiness signal. Probe initialization now installs only
  the atomic `SDL_GL_SwapBuffers` IAT owner first and waits for one forwarded
  swap to return before installing OpenGL matrix telemetry or RenderWorld hooks.
  PID 25952 separately crossed OpenVR startup and created 3400x3468 eye targets,
  then disabled stereo on the first 3D frame because the new gameplay-overlay
  draw inherited a pre-existing HPL OpenGL error. The overlay path now drains
  inherited GL errors before its own work and reports any error generated by the
  overlay itself. Release build, 38/38 CTest and the exact-build input/spatial/
  physics verifier pass. Both fixes remain **host-tested only** until the next
  real combined headset launch.
