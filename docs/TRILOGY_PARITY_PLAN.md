...
| Free-body grab/throw | Shared pose/throw policy | Consumed through BP `Grab=6` and free `Move=2`; because BP routes ordinary props such as barrels through `Move=2`, that path adapts Rework's palm-contact/throw behavior while mechanisms retain native offsets | Reliable acquisition and stable held placement |
...
| Tracked menu/UI | Shared panel/input policy | Inventory/notebook, death/full-screen pointer routes and native HUD capture mapped | Headset usability/subtitle validation |
