...
| Head rotation/camera composition | Shared transforms/camera policy | Consumed and headset exercised; current candidate transactionally publishes the eye-derived native camera position with each eye's matrices so BP billboard/beam facing uses one coherent camera state | Headset revalidate the messhall light-ray billboards plus transition/focus tests |
...
| Tracked menu/UI | Shared panel/input policy | Inventory/notebook, death/full-screen pointer routes and native HUD capture mapped; Rework-equivalent inventory drag/default/context-action routing plus the `UseItem=4` controller-aim ray and native red/green usability beam are host-tested through exact native boundaries. BP's global inventory handler discards right button `2`, so the context edge uses a narrow backend-owned direct dispatch to the existing active context/hovered widget path | Headset usability/progression, item-use targeting and subtitle validation |
