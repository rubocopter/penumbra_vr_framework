...
**You ran `Penumbra_vr.exe` from the extracted package folder.**
The package is an overlay, not a standalone game installation, and does not
contain the complete retail resource tree. Run `Install-PenumbraVR.bat` from the
package, then start SteamVR and launch Overture through Steam. Direct execution
from the package directory is not a supported test path.
