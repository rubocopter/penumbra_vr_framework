...
$frameworkRoot=[System.IO.Path]::GetFullPath((Join-Path $repoRoot '..\..'))
...
Copy-Item -LiteralPath (Join-Path $frameworkRoot 'assets\openvr\overture\actions.json') -Destination (Join-Path $package 'vr\actions.json')
