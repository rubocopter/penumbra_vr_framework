<#
.SYNOPSIS
    Generates the SteamVR controller binding files in assets/openvr/overture/bindings from a
...
param(
    ...
    # Write generated files here (defaults to the Overture-specific Framework assets).
    [string]$OutputRoot
)
...
if (-not $OutputRoot) {
    $OutputRoot = Join-Path $frameworkRoot 'assets\openvr\overture\bindings'
}
