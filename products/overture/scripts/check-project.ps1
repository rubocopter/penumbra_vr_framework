...
$openVrAssetRoot = Join-Path $frameworkRoot 'assets\openvr\overture'
$requiredPaths = @(
    'PenumbraVR.sln',
...
$requiredFrameworkPaths = @(
    'assets\openvr\overture\actions.json',
    'assets\openvr\overture\bindings\psvr2_sense.json',
    'assets\openvr\overture\bindings\vive_controller.json',
    'assets\openvr\overture\bindings\knuckles.json',
    'assets\openvr\overture\bindings\oculus_touch.json',
    'assets\openvr\overture\bindings\microsoft_motion_controller.json',
    'assets\openvr\overture\bindings\pico4_controller.json',
    'assets\openvr\overture\bindings\pico_neo3_controller.json',
    'assets\openvr\overture\bindings\holographic_controller.json',
...
$missingFrameworkPaths = foreach ($relativePath in $requiredFrameworkPaths) {
    $candidatePath = Join-Path $frameworkRoot $relativePath
    if (-not (Test-Path -LiteralPath $candidatePath)) {
        $relativePath
    }
}
if ($missingFrameworkPaths) {
    throw "Missing Framework-owned Overture files:`n - $($missingFrameworkPaths -join "`n - ")"
}
...
foreach ($relativeProjectPath in $projectPaths) {
    ...
    if ($projectText -match '(?i)penumbra_vr_rework|\.\.\\\.\.\\penumbra_vr\\') {
        throw "External Rework/Framework checkout dependency remains in $relativeProjectPath"
    }
...
$frameworkBackendPath = Join-Path $frameworkRoot 'src\backends\overture\overture_backend.cpp'
$frameworkLocomotionPath = Join-Path $frameworkRoot 'src\runtime\vr_locomotion.cpp'
$frameworkIntegrationPath = Join-Path $frameworkRoot 'src\adapters\overture_source\overture_source_integration.cpp'
foreach ($frameworkPath in @($frameworkBackendPath, $frameworkLocomotionPath, $frameworkIntegrationPath)) {
    if (-not (Test-Path -LiteralPath $frameworkPath)) {
        throw "The Framework Overture integration source is missing: $frameworkPath"
    }
}
$frameworkBackendSource = Get-Content -Raw -LiteralPath $frameworkBackendPath
$frameworkLocomotionSource = Get-Content -Raw -LiteralPath $frameworkLocomotionPath
$frameworkIntegrationSource = Get-Content -Raw -LiteralPath $frameworkIntegrationPath
if ($frameworkBackendSource -notmatch [regex]::Escape('TrackingDirectionToWorld(tracking_delta)')) {
    throw 'Physical HMD displacement must be rotated through TrackingToWorld world yaw.'
}
if ($frameworkBackendSource -notmatch 'HeadRelativeMoveDirection\s*\(\s*head_world' -or
    $frameworkLocomotionSource -notmatch 'HeadRelativeMoveDirection\s*\(\s*\r?\n?\s*const VrMatrix44& head') {
    throw 'VR locomotion direction must be derived from the world-space HMD pose.'
}
foreach ($requiredFrameworkBoundary in @(
    ...
    'SetWorldYaw(',
    'vr_tracking.GetWorldYaw()'
)) {
    if ($frameworkIntegrationSource -notmatch [regex]::Escape($requiredFrameworkBoundary)) {
        throw "The source HPL body adapter is missing '$requiredFrameworkBoundary'."
    }
}
...
if ($penumbraProjectSource -notmatch [regex]::Escape('..\..\..\src\adapters\overture_source\PenumbraVR.Framework.Overture.props')) {
    throw 'Penumbra.vcxproj must import the Framework-owned Overture props from this checkout.'
}
...
if (($actionNames | Sort-Object -Unique).Count -ne $actionNames.Count) {
    throw 'Duplicate SteamVR action names were found in assets\openvr\overture\actions.json.'
}
if (($actionSetNames | Sort-Object -Unique).Count -ne $actionSetNames.Count) {
    throw 'Duplicate SteamVR action set names were found in assets\openvr\overture\actions.json.'
}
