...
{
	mpInit = apInit;
	mpOvertureSourceIntegration = new
		penumbra_vr::adapters::overture_source::OvertureSourceIntegration();
...
{
	delete mpOvertureSourceIntegration;
...
void cPlayer::HandleFrameworkVRInput(const cVRInputState& aInputState,
	float afTimeStep, bool abGameplayActive, bool abPlayerUpdateExpected)
{
	std::string error;
	if(!mpOvertureSourceIntegration->HandleInput(
		*this, *mpInit, aInputState, afTimeStep, abGameplayActive,
		abPlayerUpdateExpected, error))
	{
		Error("Penumbra VR Framework Overture input failed: %s\n",
			error.c_str());
	}
}
...
{
	mpOvertureSourceIntegration->Reset();
	for(int i = 0; i < eVRHandIndex_Count; ++i)
	{
		ClearVRHandTarget((eVRHandIndex)i);
		mVRHeldBodies[i] = NULL;
...
void cPlayer::Update(float afTimeStep)
{
  ...
  mpVRDimmer->Update(afTimeStep);
	...
	const ePlayerState playerState = GetState();
	const bool bodyMotionEnabled =
		(playerState == ePlayerState_Normal ||
		 playerState == ePlayerState_Grab ||
		 playerState == ePlayerState_Throw ||
		 playerState == ePlayerState_Push ||
		 playerState == ePlayerState_Move ||
		 playerState == ePlayerState_WeaponMelee) &&
		!GetLookAt()->IsActive() && !mpInit->mpInventory->IsActive();
	...
	std::string frameworkError;
	if(!mpOvertureSourceIntegration->UpdatePlayer(
		*this, *mpInit, afTimeStep, bodyMotionEnabled,
		playerState == ePlayerState_Move || playerState == ePlayerState_Push,
		frameworkError))
	{
		Error("Penumbra VR Framework Overture update failed: %s\n",
			frameworkError.c_str());
	}
}
